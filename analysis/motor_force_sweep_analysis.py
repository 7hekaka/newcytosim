...
OUT = ROOT / 'analysis' / 'results' / 'motor_force_sweep'
PLOTS = OUT / 'plots'
...
MODEL_SPEC = ('fixed_global_motor_force', 'Fixed global', '#d95f02', 'o', '#d95f02')
...
def save_metric_figure(lookup: dict, metric: dict) -> Path:
    ...
    for cidx, (_, _, xlabel) in enumerate(GROUP_SPECS):
        axes[2, cidx].set_xlabel(f'Motor unbinding force (0 = no motors)\n({xlabel})', fontsize=17)
    handles = [
...
def save_window_figure(lookup: dict) -> Path:
    ...
    for cidx in range(3):
        axes[2, cidx].set_xlabel('Motor unbinding force (0 = no motors)', fontsize=17)
    handles = [
        plt.Line2D([0], [0], color=CONTROL_SPEC[2], lw=0, marker=CONTROL_SPEC[3], markersize=8, markeredgewidth=1.8, markeredgecolor=CONTROL_SPEC[2], markerfacecolor=CONTROL_SPEC[4])
    ]
    labels = ['No motors']
