...
void alsatian_xtbsvLTN6SSE(const int N, const double* A, const int lda, double* X)
{
    ...
    for ( A -= lda, --X; X > end; X -= 2 )
    {
        ...
        tt = fnmadd2(catshift1(x2, x4), mul2(ab, unpacklo2(aa, bb)), tt);
        aa = loadu2(A+3);
        bb = loadu2(B+3);
        tt = fnmadd2(x2, mul2(ab, unpackhi2(aa, bb)), tt);
        tt = fnmadd2(catshift1(x0, x2), mul2(ab, unpacklo2(aa, bb)), tt);
        aa = loadu2(A+1);
        bb = loadu2(B+1);
        ...
        tt = fnmadd2(catshift1(xx, x0), AB, tt);
        x4 = x2;
        x2 = x0;
        x0 = tt;
        storeu2(X, tt);
...
void alsatian_xtbsvLTN3(const int N, const double* pA, const int lda, double* pX)
{
    ...
    while ( pA >= end ) // for ( int j = N-3; j > 0; --j )
    {
        ...
        tt = broadcastd(a1); // save 4th position for next round
        pA -= lda;
        pX -= ORD;
    }
    // j = 0
