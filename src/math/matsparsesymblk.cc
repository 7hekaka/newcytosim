 ...
 */
void MatrixSparseSymmetricBlock::Column::sortElements(Element tmp[], size_t tmp_size)
{
...
bool MatrixSparseSymmetricBlock::prepareForMultiply(int)
{
    next_[size_] = size_;
...
void MatrixSparseSymmetricBlock::Column::vecMulAdd3D_SSE(const real* X, real* Y, size_t jj) const
{
    assert_true(size_ > 0);
    assert_true(inx_[0] == jj);
...
void MatrixSparseSymmetricBlock::Column::vecMulAdd3D_SSEU(const real* X, real* Y, size_t jj) const
{
    assert_true(size_ > 0);
    assert_true(inx_[0] == jj);
#if ( BLOCK_SIZE == 3 ) && ( REAL_IS_DOUBLE == 0 )
...
void MatrixSparseSymmetricBlock::Column::vecMulAdd3D_AVXU(const real* X, real* Y, size_t jj) const
{
    ...
    // load 3x3 matrix element into 3 vectors:
    {
        ...
        sa = mul4(streamload4(M  ), tt);
        sb = mul4(streamload4(M+4), tt);
        sc = mul4(streamload4(M+8), tt);
...
void MatrixSparseSymmetricBlock::vecMul(const real* X, real* Y) const
{
    zero_real(size_, Y);
