...
void Tubule::setInteractionsA(Meca& meca) const
{
    ...
    for ( index_t i = 0; i < e; ++i )
    {
        ...
        for ( index_t n = 0; n < NFIL; ++n )
        {
            ...
            meca.addSideLinkS(fil_[n], i, 0, Mecapoint(fil_[n+1],i), leg, stiff);
        }
    }
    ...
    for ( index_t n = 0; n < NFIL; ++n )
    {
        ...
        meca.addSideLinkS(fil_[n], e-1, 1, Mecapoint(fil_[n+1],e), leg, stiff);
    }
#endif
}
...
void Tubule::setInteractions(Meca& meca) const
{
    ...
    for ( index_t i = 0; i < e; ++i )
    {
        ...
        for ( index_t n = 0; n < NFIL; ++n )
        {
            ...
            // orthoradial beams:
            meca.addSideLinkS(fil_[n], i, 0, Mecapoint(fil_[n+1],i), mat.vecmul(leg), stiffL);
            // radial spoke:
            meca.addSideLinkS(fil_[n], i, 0, Mecapoint(bone_,i), beta*cross(dir,leg), stiffR);
            // twist stiffness
    ...
    for ( index_t n = 0; n < NFIL; ++n )
    {
        ...
        meca.addSideLinkS(fil_[n], e-1, 1, Mecapoint(fil_[n+1],e), mat.vecmul(leg), stiffL);
        meca.addSideLinkS(fil_[n], e-1, 1, Mecapoint(bone_,e), beta*cross(dir,leg), stiffR);
    }
#endif
}
...
void Tubule::setInteractionsC(Meca& meca) const
{
    ...
    for ( index_t i = 0; i < e; ++i )
    {
        ...
        for ( index_t n = 0; n < NFIL; ++n )
        {
            ...
            meca.addSideLinkS(fil_[n], i, 0, Mecapoint(fil_[n+1],i), leg, stiffL);
    ...
    for ( index_t n = 0; n < NFIL; ++n )
    {
        ...
        meca.addSideLinkS(fil_[n], e-1, 1, Mecapoint(fil_[n+1],e), leg, stiffL);
    }
#endif
}
...
void Tubule::setInteractionsD(Meca& meca) const
{
    ...
    for ( index_t n = 0; n < NFIL; ++n )
    {
        ...
        for ( index_t i = 0; i < e; ++i )
        {
            ...
            meca.addSideLinkS(fib, i, 0, Mecapoint(fil_[n+2],i), leg, stiffL);
        ...
        meca.addSideLinkS(fib, e-1, 1, Mecapoint(fil_[n+2],e), leg, stiffL);
    }
#endif
}
