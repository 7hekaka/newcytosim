                    ...
                    const real weight)
{
    ...
    if ( modulo )
    {
        Vector off = interpolate3(vPTS, pts, cc1, cc2, cc3) - interpolate1(vPTS, ii0);
        off = modulo->offset(off);
        if ( off.is_not_zero() )
        {
            add_base(ii0, off, ww0);
                    ...
                    const real weight)
{
    ...
    if ( modulo )
    {
        Vector off = interpolate2(vPTS, ii0, cc0, cc1) + interpolate3(vPTS, pts, cc2, cc3, cc4);
        off = modulo->offset(off);
        if ( off.is_not_zero() )
        {
            add_base(ii0, off, ww0);
                    ...
                    const real weight)
{
    ...
    if ( modulo )
    {
        Vector off = interpolate4(vPTS, pts, cc1, cc2, cc3, cc4) - interpolate1(vPTS, ii0);
        off = modulo->offset(off);
        if ( off.is_not_zero() )
        {
            add_base(ii0, off, ww0);
                    ...
                    const real weight)
{
    ...
    if ( modulo )
    {
        Vector off = interpolate2(vPTS, ii0, cc0, cc1) + interpolate4(vPTS, pts, cc2, cc3, cc4, cc5);
        off = modulo->offset(off);
        if ( off.is_not_zero() )
        {
            add_base(ii0, off, ww0);
                        ...
                        const real weight)
{
    ...
    const size_t aa = ptA.matIndex();  // coef is +weight
    const size_t bb = ptB.matIndex();  // coef is -weight
    assert_true( aa != bb );
                        ...
                        const real weight)
{
    ...
    Vector axi = interpolate4(vPTS, inx, cc2, cc3, cc4, cc5) + interpolate2(vPTS, ii0, cc0, cc1);
    Vector off;
                       ...
                       const real weight)
{
    ...
    // indices:
    const size_t ii0 = segA.matIndex();
                        ...
                        const real weight)
{
    ...
    size_t P = ptB.point();
    if ( P > 0 )
        addSideLink3D(ptA, Interpolation(ptB.mecable(), 1.0, P-1, P), arm, weight);
    else
        addSideLink3D(ptA, Interpolation(ptB.mecable(), 0.0, P, P+1), arm, weight);
...
void Meca::addCoulomb(Mecapoint const& ptA, Mecapoint const& ptB, real weight)
{
    ...
    const size_t aa = ptA.matIndex();
    const size_t bb = ptB.matIndex();
