 ...
 */
void SpacePolygonZ::setConfinement(Vector const& pos, Mecapoint const& pe, Meca& meca, real stiff) const
{
    //Space::setConfinement(pos, pe, meca, stiff); return;
#if ( DIM > 2 )
    real P, R = pos.normXY();
    size_t hit;
...
void SpacePolygonZ::setConfinement(Vector const& pos, Mecapoint const& pe, real rad, Meca& meca, real stiff) const
{
...
void SpacePolygonZ::draw3D() const
{
    ...
    for ( size_t n = 0; n < npts; n++ )
    {
        ...
        if (( R1 >= 0 ) & ( R2 >= 0 ))
        {
            flute6 * flu = gle::mapBufferN3V3(2+2*gle::pi_twice);
            size_t i = 0;
...
void SpacePolygonZ::drawRings() const
{
    ...
    for ( size_t n = 0; n < npts; n++ )
    {
        ...
        {
            float nX(pts[n].dy), nY(-pts[n].dx);
            flute6 * flu = gle::mapBufferN3V3(2+gle::pi_twice);
            size_t i = 0;
            for ( size_t j = 0; j <= gle::pi_twice; ++j )
            {
                float C = gle::cos_(j), S = gle::sin_(j);
