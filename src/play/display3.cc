...
inline void Display3::drawPoint(Vector const& pos, PointDisp const* dis) const
{
    ...
    {
        assert_true(glIsEnabled(GL_LIGHTING));
        drawObject(pos, scale(dis->size), gle::sphere1);
    }
}
...
inline void Display3::drawHand(Vector const& pos, PointDisp const* dis) const
{
    if ( dis->perceptible )
    {
        assert_true(glIsEnabled(GL_LIGHTING));
        dis->color.load_both();
        drawObject(pos, scale(dis->size), gle::blob);
    }
}
...
void Display3::drawFiberLines(Fiber const& fib, int style) const
{
    ...
    const real rad = scale(disp->line_width);
...
{
    FiberDisp const*const disp = fib.prop->disp;
    float rad = scale(disp->speckle_size);
...
void Display3::drawFiberPoints(Fiber const& fib) const
{
    ...
    const GLfloat rad = scale(disp->point_size);
...
void Display3::drawOrganizer(Organizer const& obj) const
{
    ...
    if ( disp && ( disp->style & 2 ))
    {
        ...
        const float wid = scale(disp->width);
...
void Display3::drawSingleB(Single const* obj) const
{
    ...
    GLfloat wid = scale(disp->width);
    GLfloat rad = scale(disp->size);
...
void Display3::drawCoupleBplain(Couple const* cx) const
{
    ...
    pd1->color.load_both();
    stretchTube(p1, p2, scale(pd1->width), gle::hexTube);
...
void Display3::drawCoupleBside(Couple const* cx) const
{
    ...
    if ( pd1 == pd2 )
    {
        ...
        drawPoint(mid, scale(pd1->width));
        stretchTube(p2, mid, scale(pd2->width), gle::hexTube);
        stretchTube(p1, mid, scale(pd1->width), gle::hexTube);
        drawPoint(p1, pd1);
        drawPoint(p2, pd2);
        return;
    }
...
void Display3::drawCoupleBalt(Couple const* cx) const
{
    ...
    if ( cx->fiber1()->disp->color.transparent() )
    {
        ...
        glDepthMask(GL_FALSE);
        stretchTube(p1, p2, scale(pd2->width), gle::hexTube);
        glDepthMask(GL_TRUE);
        return;
    }
#endif
